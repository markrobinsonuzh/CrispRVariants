...
cigars <- c("5M5D20M","10M10D20M","5M10D20M","5M20D10M","1M3I2M3I3M10D10M",
            ...
            "4S3M1I6M4D8M")
wdths <- cigarWidthAlongPairwiseSpace(cigars)
gr <- GRanges("chr1", IRanges(start = 1, width = wdths), cigar = cigars, strand = "+")
galns <- as(gr, "GAlignments")
target <- GRanges("chr1", IRanges(11, 20))
...
# Test 2
quals <- DNAStringSet(sapply(wdths, function(x) paste0(rep("H",x), collapse = ""))) 
...
# Test 3
seqs <- DNAStringSet(sapply(wdths, function(x) paste0(rep("A",x), collapse = ""))) 
